...
<div class="flex items-center justify-between mb-6">
    ...
    <div class="flex gap-2">
        <a href="/admin/guru/import-template" class="bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded-lg text-sm font-medium flex items-center gap-2">
...
<div id="bulkBar" class="hidden mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg flex flex-wrap items-center gap-3">
    ...
    <form method="POST" action="/admin/guru/bulk-delete" onsubmit="return confirm('Hapus semua yang dipilih?')" class="inline">
        <?= csrf_field() ?>
    ...
    <form method="POST" action="/admin/guru/bulk-reset-password" onsubmit="return confirm('Reset password semua yang dipilih?')" class="inline">
        <?= csrf_field() ?>
        <div class="bulkIds"></div>
...
<div class="bg-white rounded-xl shadow p-4">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <div class="flex items-center gap-2">
            ...
            <select id="tableGuru_perpage" class="border border-gray-300 rounded px-2 py-1 text-sm">
        ...
        <input id="tableGuru_search" type="text" placeholder="Cari guru..." class="border border-gray-300 rounded-lg px-3 py-1.5 text-sm w-full sm:w-64 focus:outline-none focus:ring-2 focus:ring-blue-400">
    </div>
    ...
    <div class="overflow-x-auto">
        <table id="tableGuru" class="w-full text-sm">
            ...
            <tbody>
                <?php if (empty($gurus)): ?>
                <tr><td colspan="6" class="text-center py-8 text-gray-400">Belum ada data guru.</td></tr>
                <?php else: ?>
                <?php foreach ($gurus as $i => $g): ?>
                <tr class="border-t hover:bg-gray-50">
                    <td class="p-3"><input type="checkbox" class="row-check rounded" value="<?= $g['id'] ?>"></td>
                    <td class="p-3"><?= $i + 1 ?></td>
                    <td class="p-3 font-mono text-xs"><?= esc($g['nik']) ?></td>
                    <td class="p-3 font-medium"><?= esc($g['nama']) ?></td>
                    <td class="p-3 text-gray-500"><?= esc($g['email']) ?></td>
                    <td class="p-3 text-center whitespace-nowrap">
                        <button onclick='openEditModal(<?= json_encode($g) ?>)' class="bg-yellow-500 hover:bg-yellow-600 text-white px-2 py-1 rounded text-xs mr-1">
                            <i class="fas fa-edit"></i>
                        </button>
                        <form method="POST" action="/admin/guru/reset-password/<?= $g['id'] ?>" class="inline" onsubmit="return confirm('Reset password guru ini?')">
                            <?= csrf_field() ?>
                            <button type="submit" class="bg-orange-500 hover:bg-orange-600 text-white px-2 py-1 rounded text-xs mr-1">
                                <i class="fas fa-key"></i>
                            </button>
                        </form>
                        <form method="POST" action="/admin/guru/destroy/<?= $g['id'] ?>" class="inline" onsubmit="return confirm('Hapus guru ini?')">
                            <?= csrf_field() ?>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mt-4 gap-2">
        <span id="tableGuru_info" class="text-sm text-gray-500"></span>
        <div id="tableGuru_pagination" class="flex gap-1 flex-wrap"></div>
    </div>
</div>
        ...
        <form method="POST" action="/admin/guru/store">
            ...
            <div class="grid grid-cols-2 gap-3 mb-3">
                <div class="col-span-2">
                    ...
                    <input type="text" name="nik" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" required>
                </div>
                <div class="col-span-2">
                ...
                <div class="col-span-2">
                    ...
                    <input type="email" name="email" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                </div>
                <div class="col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Password <span class="text-gray-400 font-normal">(kosong = default: Guru@MTsN2026)</span></label>
    ...
    <div class="modal-card bg-white rounded-xl shadow-xl w-full max-w-md p-6">
        ...
        <form method="POST" id="formEdit" action="">
            ...
            <div class="space-y-3 mb-3">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">NIK</label>
                    <input type="text" name="nik" id="editNik" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nama Lengkap</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                    <input type="email" name="email" id="editEmail" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                </div>
                <div>
...
<div id="modalImport" class="modal-overlay hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
    <div class="modal-card bg-white rounded-xl shadow-xl w-full max-w-sm p-6">
        ...
        <form method="POST" action="/admin/guru/import-excel" enctype="multipart/form-data">
            <?= csrf_field() ?>
            <div class="mb-4">
                ...
                <p class="text-xs text-gray-400 mt-1">Gunakan template yang tersedia. Kolom: NIK, Nama, Email, Password.</p>
            </div>
            <div class="flex justify-end gap-2">
...
function openEditModal(data) {
    ...
    document.getElementById('formEdit').action = '/admin/guru/update/' + data.id;
    openModal('modalEdit');
}
