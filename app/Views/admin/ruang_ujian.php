...
<div id="bulkBar" class="hidden mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg flex items-center gap-3">
    ...
    <form method="POST" action="/admin/ruang-ujian/bulk-delete" onsubmit="return confirm('Hapus semua yang dipilih?')">
        <?= csrf_field() ?>
        <div id="bulkIds"></div>
...
<div class="bg-white rounded-xl shadow p-4">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        ...
        <input id="tableRuang_search" type="text" placeholder="Cari ujian..." class="border border-gray-300 rounded-lg px-3 py-1.5 text-sm w-full sm:w-64 focus:outline-none focus:ring-2 focus:ring-blue-400">
    </div>
    ...
    <div class="overflow-x-auto">
        <table id="tableRuang" class="w-full text-sm">
            ...
            <tbody>
                <?php if (empty($ruang)): ?>
                <tr><td colspan="12" class="text-center py-8 text-gray-400">Belum ada ruang ujian.</td></tr>
                <?php else: ?>
                ...
                <tr class="border-t hover:bg-gray-50">
                    ...
                    <td class="p-3 text-gray-600 text-xs"><?= esc($r['nama_guru'] ?? '-') ?></td>
                    ...
                    <td class="p-3 text-center">
                        <div class="flex items-center justify-center gap-1">
                            ...
                            <button onclick="copyToken('<?= esc($r['token']) ?>')" class="text-gray-400 hover:text-blue-600" title="Salin token">
                                <i class="fas fa-copy text-xs"></i>
                            </button>
                        </div>
                    </td>
                    <td class="p-3 text-center whitespace-nowrap">
                        <a href="/admin/ruang-ujian/monitoring/<?= $r['id'] ?>" class="bg-blue-500 hover:bg-blue-600 text-white px-2 py-1 rounded text-xs mr-1">
                        ...
                        <form method="POST" action="/admin/ruang-ujian/regenerate-token/<?= $r['id'] ?>" class="inline" onsubmit="return confirm('Buat token baru?')">
                            <?= csrf_field() ?>
                            <button type="submit" class="bg-purple-500 hover:bg-purple-600 text-white px-2 py-1 rounded text-xs mr-1" title="Regenerate Token">
                                <i class="fas fa-sync"></i>
                            </button>
                        </form>
                        <form method="POST" action="/admin/ruang-ujian/destroy/<?= $r['id'] ?>" class="inline" onsubmit="return confirm('Hapus ruang ujian ini?')">
                            <?= csrf_field() ?>
...
<div id="modalTambah" class="modal-overlay hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-start justify-center pt-8 pb-8 overflow-y-auto">
    <div class="modal-card bg-white rounded-xl shadow-xl w-full max-w-2xl mx-4 p-6">
        ...
        <form method="POST" action="/admin/ruang-ujian/store">
            <?= csrf_field() ?>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nama Ujian</label>
                    <input type="text" name="nama_ujian" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Bank Soal</label>
                    <select name="bank_soal_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                ...
                <div>
                    ...
                    <select name="mapel_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Guru Pengawas</label>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="tanggal_mulai" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Selesai</label>
                    <input type="datetime-local" name="tanggal_selesai" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Durasi (menit)</label>
                    <input type="number" name="durasi" value="90" min="1" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                    <select name="status" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                ...
                <div>
                    ...
                    <input type="number" name="max_login" value="1" min="1" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Batas Keluar (kali)</label>
                    <input type="number" name="batas_keluar" value="3" min="0" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                </div>
                <div class="flex items-center gap-4">
                    ...
                    <label class="flex items-center gap-2 cursor-pointer">
                        ...
                        <span class="text-sm text-gray-700">Acak Jawaban</span>
                    </label>
                </div>
            </div>
    ...
    <div class="modal-card bg-white rounded-xl shadow-xl w-full max-w-2xl mx-4 p-6">
        ...
        <form method="POST" id="formEdit" action="">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nama Ujian</label>
                    <input type="text" name="nama_ujian" id="editNamaUjian" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Bank Soal</label>
                    <select name="bank_soal_id" id="editBankSoalId" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                        <option value="">-- Pilih Bank Soal --</option>
                        <?php foreach ($bank_soal as $b): ?>
                        <option value="<?= $b['id'] ?>"><?= esc($b['nama_bank']) ?></option>
                        <?php endforeach; ?>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Mata Pelajaran</label>
                    <select name="mapel_id" id="editMapelId" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                        <option value="">-- Pilih Mapel --</option>
                        <?php foreach ($mapels as $m): ?>
                        <option value="<?= $m['id'] ?>"><?= esc($m['nama_mapel']) ?></option>
                        <?php endforeach; ?>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Guru Pengawas</label>
                    <select name="guru_id" id="editGuruId" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                        <option value="">-- Pilih Guru --</option>
                        <?php foreach ($gurus as $g): ?>
                        <option value="<?= $g['id'] ?>"><?= esc($g['nama']) ?></option>
                        <?php endforeach; ?>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Mulai</label>
                    <input type="datetime-local" name="tanggal_mulai" id="editTanggalMulai" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Selesai</label>
                    <input type="datetime-local" name="tanggal_selesai" id="editTanggalSelesai" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Durasi (menit)</label>
                    <input type="number" name="durasi" id="editDurasi" min="1" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                    <select name="status" id="editStatus" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                        <option value="draft">Draft</option>
                        <option value="aktif">Aktif</option>
                        <option value="selesai">Selesai</option>
                    </select>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Max Login</label>
                    <input type="number" name="max_login" id="editMaxLogin" min="1" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Batas Keluar (kali)</label>
                    <input type="number" name="batas_keluar" id="editBatasKeluar" min="0" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                </div>
                <div class="flex items-center gap-4">
                    <label class="flex items-center gap-2 cursor-pointer">
...
function openEditModal(data) {
    ...
    document.getElementById('editDurasi').value       = data.durasi;
    document.getElementById('editStatus').value       = data.status;
    document.getElementById('editMaxLogin').value     = data.max_login;
    document.getElementById('editBatasKeluar').value  = data.batas_keluar;
    document.getElementById('editAcakSoal').checked   = data.acak_soal == 1;
    document.getElementById('editAcakJawaban').checked = data.acak_jawaban == 1;
    // Format datetime-local
    if (data.tanggal_mulai) {
        document.getElementById('editTanggalMulai').value = data.tanggal_mulai.replace(' ', 'T').substring(0, 16);
    }
    if (data.tanggal_selesai) {
        document.getElementById('editTanggalSelesai').value = data.tanggal_selesai.replace(' ', 'T').substring(0, 16);
    }
    document.getElementById('formEdit').action = '/admin/ruang-ujian/update/' + data.id;
    openModal('modalEdit');
}

function copyToken(token) {
    navigator.clipboard.writeText(token).then(() => {
        if (typeof showToast === 'function') showToast('Token disalin: ' + token, 'success');
        else alert('Token disalin: ' + token);
    });
}
