...
<div id="bulkBar" class="hidden mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg flex items-center gap-3">
    ...
    <form method="POST" action="/admin/bank-soal/bulk-delete" onsubmit="return confirm('Hapus semua yang dipilih?')">
        <?= csrf_field() ?>
        <div id="bulkIds"></div>
...
<div class="bg-white rounded-xl shadow p-4">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        ...
        <input id="tableBankSoal_search" type="text" placeholder="Cari bank soal..." class="border border-gray-300 rounded-lg px-3 py-1.5 text-sm w-full sm:w-64 focus:outline-none focus:ring-2 focus:ring-blue-400">
    </div>
    ...
    <div class="overflow-x-auto">
        <table id="tableBankSoal" class="w-full text-sm">
            ...
            <tbody>
                ...
                <tr><td colspan="8" class="text-center py-8 text-gray-400">Belum ada data bank soal.</td></tr>
                <?php else: ?>
                ...
                <tr class="border-t hover:bg-gray-50">
                    ...
                    <td class="p-3 text-gray-600"><?= esc($b['nama_guru'] ?? '-') ?></td>
                    <td class="p-3 text-center">
                        <span class="bg-gray-100 text-gray-700 px-2 py-0.5 rounded text-xs font-medium"><?= $b['jml_soal'] ?? 0 ?></span>
                    </td>
                    <td class="p-3 text-center whitespace-nowrap">
                        <a href="/admin/soal/<?= $b['id'] ?>" class="bg-blue-500 hover:bg-blue-600 text-white px-2 py-1 rounded text-xs mr-1">
                        ...
                        <form method="POST" action="/admin/bank-soal/destroy/<?= $b['id'] ?>" class="inline" onsubmit="return confirm('Hapus bank soal ini?')">
                            <?= csrf_field() ?>
...
<div id="modalTambah" class="modal-overlay hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
    <div class="modal-card bg-white rounded-xl shadow-xl w-full max-w-md p-6">
        ...
        <form method="POST" action="/admin/bank-soal/store">
            <?= csrf_field() ?>
            <div class="space-y-3 mb-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nama Bank Soal</label>
                    <input type="text" name="nama_bank" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" required>
                </div>
                <div>
    ...
    <div class="modal-card bg-white rounded-xl shadow-xl w-full max-w-md p-6">
        ...
        <form method="POST" id="formEdit" action="">
            ...
            <div class="space-y-3 mb-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nama Bank Soal</label>
                    <input type="text" name="nama_bank" id="editNamaBank" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Mata Pelajaran</label>
...
function openEditModal(data) {
    ...
    document.getElementById('formEdit').action    = '/admin/bank-soal/update/' + data.id;
    openModal('modalEdit');
}
