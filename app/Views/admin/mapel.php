...
<div id="bulkBar" class="hidden mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg flex items-center gap-3">
    ...
    <form method="POST" action="/admin/mapel/bulk-delete" onsubmit="return confirm('Hapus semua yang dipilih?')">
        <?= csrf_field() ?>
        <div id="bulkIds"></div>
...
<div class="bg-white rounded-xl shadow p-4">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <div class="flex items-center gap-2">
            ...
            <select id="tableMapel_perpage" class="border border-gray-300 rounded px-2 py-1 text-sm">
        ...
        <input id="tableMapel_search" type="text" placeholder="Cari mapel..." class="border border-gray-300 rounded-lg px-3 py-1.5 text-sm w-full sm:w-64 focus:outline-none focus:ring-2 focus:ring-blue-400">
    </div>

    <div class="overflow-x-auto">
        <table id="tableMapel" class="w-full text-sm">
            <thead>
            ...
            <tbody>
                <?php if (empty($mapels)): ?>
                <tr><td colspan="4" class="text-center py-8 text-gray-400">Belum ada data mapel.</td></tr>
                <?php else: ?>
                <?php foreach ($mapels as $i => $m): ?>
                <tr class="border-t hover:bg-gray-50">
                    <td class="p-3"><input type="checkbox" class="row-check rounded" value="<?= $m['id'] ?>"></td>
                    <td class="p-3"><?= $i + 1 ?></td>
                    <td class="p-3 font-medium"><?= esc($m['nama_mapel']) ?></td>
                    <td class="p-3 text-center">
                        <button onclick='openEditModal(<?= json_encode($m) ?>)' class="bg-yellow-500 hover:bg-yellow-600 text-white px-2 py-1 rounded text-xs mr-1">
                            <i class="fas fa-edit"></i> Edit
                        </button>
                        <form method="POST" action="/admin/mapel/destroy/<?= $m['id'] ?>" class="inline" onsubmit="return confirm('Hapus mapel ini?')">
                            <?= csrf_field() ?>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mt-4 gap-2">
        <span id="tableMapel_info" class="text-sm text-gray-500"></span>
        <div id="tableMapel_pagination" class="flex gap-1"></div>
    </div>
</div>
...
<div id="modalTambah" class="modal-overlay hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
    <div class="modal-card bg-white rounded-xl shadow-xl w-full max-w-md p-6">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold">Tambah Mapel</h3>
            <button onclick="closeModal('modalTambah')" class="text-gray-400 hover:text-gray-600"><i class="fas fa-times text-xl"></i></button>
        </div>
        <form method="POST" action="/admin/mapel/store">
            <?= csrf_field() ?>
            <div id="mapelInputs">
                <div class="flex gap-2 mb-2">
                    <input type="text" name="nama_mapel[]" placeholder="Nama mapel..." class="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" required>
                ...
                <div class="flex gap-2 mb-2">
                    <input type="text" name="nama_mapel[]" placeholder="Nama mapel..." class="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                    <button type="button" onclick="removeRow(this)" class="text-red-400 hover:text-red-600 px-2"><i class="fas fa-minus-circle"></i></button>
                </div>
                <div class="flex gap-2 mb-2">
                    <input type="text" name="nama_mapel[]" placeholder="Nama mapel..." class="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
            ...
            <button type="button" onclick="addMapelRow()" class="text-blue-600 hover:text-blue-800 text-sm mb-4 flex items-center gap-1">
...
<div id="modalEdit" class="modal-overlay hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
    <div class="modal-card bg-white rounded-xl shadow-xl w-full max-w-sm p-6">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold">Edit Mapel</h3>
        ...
        <form method="POST" id="formEdit" action="">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama_mapel" id="editNamaMapel" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" required>
            </div>
            <div class="flex justify-end gap-2">
...
function openEditModal(data) {
    ...
    document.getElementById('formEdit').action = '/admin/mapel/update/' + data.id;
    openModal('modalEdit');
}
...
function addMapelRow() {
    ...
    div.innerHTML = `<input type="text" name="nama_mapel[]" placeholder="Nama mapel..." class="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
        <button type="button" onclick="removeRow(this)" class="text-red-400 hover:text-red-600 px-2"><i class="fas fa-minus-circle"></i></button>`;
    container.appendChild(div);
}

function removeRow(btn) {
    const rows = document.querySelectorAll('#mapelInputs .flex');
